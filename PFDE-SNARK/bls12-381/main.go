...
// rangeCheckJubjubFr enforces 0 <= v <= r-1 where r is the Jubjub scalar modulus.
...
func rangeCheckJubjubFr(api frontend.API, v frontend.Variable) error {
	curve, err := ted.NewEdCurve(api, gcTed.BLS12_381)
	if err != nil {
		return err
	}
...
func (c *Circuit) Define(api frontend.API) error {
	// Range check for SK: SK < q (Jubjub curve order)
	if err := rangeCheckJubjubFr(api, c.SK); err != nil {
		return err
	}
