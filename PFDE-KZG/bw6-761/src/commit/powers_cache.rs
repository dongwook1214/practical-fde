...
mod test {
    ...
    fn cache_roundtrip_and_resume() {
        ...
        let mut cache = PowersCache::<BW6_761>::open_or_create(&root, tau, 4).unwrap();
    ...
    fn cache_writes_uncompressed_chunks_and_reads_legacy_compressed_chunks() {
        ...
        let mut manifest = PowersCacheManifest::<BW6_761>::new(tau, 4);
        manifest.generated = 6;
        fs::create_dir_all(&root).unwrap();
        write_manifest(&root, &manifest).unwrap();

        let first_chunk = Powers::<BW6_761>::unsafe_setup_from(tau, 0, 4).unwrap();
        let second_chunk = Powers::<BW6_761>::unsafe_setup_from(tau, 4, 2).unwrap();
        ...
        let expected_chunk = Powers::<BW6_761>::unsafe_setup_from(tau, 4, 4).unwrap();
