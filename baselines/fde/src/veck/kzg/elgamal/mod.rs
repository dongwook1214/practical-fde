...
use crate::veck::kzg::elgamal::divide::divide_dense_poly_fast;
    ...
    D: Digest + Clone + Send + Sync,
{
    pub fn new_v2<R: Rng>(
        ...
        let domain_size = encryption_proof.range_proofs.len();
        ...
        let (f_q_poly, remainder) = divide_dense_poly_fast(&(f_poly - f_s_poly), vanishing_poly);
    ...
    pub fn verify_v2(
        ...
        let domain_size = self.encryption_proof.range_proofs.len();
        let domain = GeneralEvaluationDomain::<C::ScalarField>::new(domain_size)
            .ok_or(CrateError::InvalidFftDomain(domain_size))?;
